...
use crate::{
    ...
    primitives::{
        ...
        SpecId::{AMSTERDAM, CANCUN, PRAGUE, SHANGHAI},
...
pub fn deduct_caller_inner<SPEC: Spec>(caller_account: &mut Account, env: &Env) {
    ...
    let gas_cost = if SPEC::enabled(AMSTERDAM) {
        let gas_cost_parts = env.effective_gas_prices().expect("already checked")
            * env.tx.gas_limits.expect("already checked");
        U256::from(gas_cost_parts.sum())
    } else {
